...
class CountryCatalogue:
    ...
    def remove(self, country: Country) -> Country:
        ...
        return country if country in self.catalogue else ValueError
    ...
    def most_populous_continent(self) -> str:
        ...
        if len(self.catalogue) > 0:
          continent_dict = {}
          for country in self.catalogue:
            if country.continent not in continent_dict:
              continent_dict[country.continent] = 0
            else:
              continent_dict[country.continent] += 1
          
          most_occurance = max(continent_dict.values())
          list_of_continents = list(continent_dict.keys())

          index_of_most_populous_continent = list(continent_dict.values()).index(most_occurance)
