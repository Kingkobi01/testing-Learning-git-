...
def convert_degrees_to_decimal(degrees: float, minutes: float, seconds: float) -> float:
    ...
    minutes_degrees = minutes/60
    seconds_degrees = seconds/ 3600

    return (degrees + minutes_degrees + seconds_degrees)
...
def subtended_area(
    ...
    **Warning:** Radians are typically a nicer unit to work with compared to degrees; humans tend to like degrees, but
    ...
    lat_1 = math.radians(latitude_line_1)
    lat_2 = math.radians(latitude_line_2)

    area : float = (math.pi/180) * (EARTH_RADIUS ** 2) * abs((math.sin(lat_1) - math.sin(lat_2))  ) * abs((longitude_line_1 - longitude_line_2)  )
    return area
...
def number_starbucks_within_area(
    ...
    number_of_starbucks: int = 0

    lat_1 = latitude_line_1
    lat_2 = latitude_line_2
    ...
    for starbucks in locations:
        ...
        if (lat_1 < latitude < lat_2) and (longitude_line_1< longitude < longitude_line_2):
           number_of_starbucks += 1
...
def starbucks_per_square_kilometer(
    ...
    locations = load_starbucks_data("starbucks2018.csv")

    area = subtended_area(latitude_line_1, latitude_line_2, longitude_line_1, longitude_line_2)

    number_of_starbucks_within_the_subtended_area = number_starbucks_within_area(locations, latitude_line_1, latitude_line_2, longitude_line_1, longitude_line_2)

    density = number_of_starbucks_within_the_subtended_area / area
